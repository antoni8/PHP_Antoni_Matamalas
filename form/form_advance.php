...
if ($_GET){
  ...
  if(isset($email)){
    if(!emailValidation($email)){
      $emailValidationMessage = "Email don't works!";}
  }
  //password validation
  $pwd=$_GET['pwd'];
  ...
  if((isset($pwd))&&(isset($pwdConfirm))){
    if(!pwdValidation($pwd, $pwdConfirm)){
      $pwdValidationMessage="<b>Passwords don't match</b> Type them again, please.";}
    else {
      $pwdValidationMessage="<b>Passwords match!";}
  }
...
<body>
  ...
  <FORM ACTION="<?php echo $_SERVER["PHP_SELF"];?>" METHOD="GET">
    ...
    if (isset($fullName)){
      ...
      foreach($fullNameSplit as $x){//complete the foreach loop to show the split fullName in parts
        echo $x . "<br>";
      }
    }
    ...
    if (isset($pwdValidationMessage))
      echo 'El hash és: ' . $pwdValidationMessage . '<br>';
